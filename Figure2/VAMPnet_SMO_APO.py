...
#npys = list(natsorted(lsext('./57npy_14940/','.npy')[0] + lsext('./57npy_14952/','.npy')[0]))

filename = []
#for npy in npys:
#    totdist.append(np.load(npy))
#    filename.append(npy)
totdist = np.load('./totdist_RRCSg40.npy',allow_pickle=True) 
#pickle.dump(filename, open(f'./traj_filenames_SMO_APO_{num_meta}.pkl','wb'))
totdist_new = []
for dist in totdist:
...
pickle.dump(state_prob,open(f'SMO_APO_prob_{num_meta}.pkl','wb'))


Inac_distances = np.load('./Inactive_RRCS_distances_SMO_APO.npy')
Ac_distances = np.load('./Active_RRCS_distances_SMO_APO.npy')

Inac_prob = model.transform(Inac_distances)
Ac_prob = model.transform(Ac_distances)

pickle.dump(Inac_prob,open(f'SMO_APO_prob_Inac_{num_meta}.pkl','wb'))
pickle.dump(Ac_prob,open(f'SMO_APO_prob_Ac_{num_meta}.pkl','wb'))



pickle.dump(vampnet.train_scores,open(f'SMO_APO_training_score_{num_meta}.pkl','wb'))

pickle.dump(vampnet.validation_scores,open(f'SMO_APO_val_score_{num_meta}.pkl','wb'))

pickle.dump(model, open(f'./SMO_APO_model_{num_meta}.pkl','wb'))
