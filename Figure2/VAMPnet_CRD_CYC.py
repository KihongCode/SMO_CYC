...
for npy in npys:
    ...
    filename.append(npy)
pickle.dump(filename, open('./traj_filenames_CRD_CYC.pkl','wb'))
totdist_new = []
for dist in totdist:
...
#pickle.dump(state_prob,open('CYC_prob.pkl','wb'))


Inac_distances = np.load('./Sys8_HMass_Distances.npy')
Ac_distances = np.load('./Sys2_HMass_Distances.npy')

Inac_prob = model.transform(Inac_distances)
Ac_prob = model.transform(Ac_distances)

#pickle.dump(Inac_prob,open('CYC_prob_Inac.pkl','wb'))
#pickle.dump(Ac_prob,open('CYC_prob_Ac.pkl','wb'))



#pickle.dump(vampnet.train_scores,open('CYC_training_score.pkl','wb'))

#pickle.dump(vampnet.validation_scores,open('CYC_val_score.pkl','wb'))

#pickle.dump(model, open('./CRD_CYC_model.pkl','wb'))
