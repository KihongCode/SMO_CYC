...
for npy in npys:
    ...
    filename.append(npy)
pickle.dump(filename, open(f'./traj_filenames_TMD_CYC_{num_meta}.pkl','wb'))
totdist_new = []
for dist in totdist:
...
pickle.dump(state_prob,open(f'TMD_CYC_prob_{num_meta}.pkl','wb'))


Inac_distances = np.load('./Sys1_HMass_Distances.npy')
Ac_distances = np.load('./Sys9_HMass_Distances.npy')

Inac_prob = model.transform(Inac_distances)
Ac_prob = model.transform(Ac_distances)

pickle.dump(Inac_prob,open(f'TMD_CYC_prob_Inac_{num_meta}.pkl','wb'))
pickle.dump(Ac_prob,open(f'TMD_CYC_prob_Ac_{num_meta}.pkl','wb'))



pickle.dump(vampnet.train_scores,open(f'TMD_CYC_training_score_{num_meta}.pkl','wb'))

pickle.dump(vampnet.validation_scores,open(f'TMD_CYC_val_score_{num_meta}.pkl','wb'))

pickle.dump(model, open(f'./TMD_TMD_CYC_model_{num_meta}.pkl','wb'))
