...
data1 = np.load('Sys3A_HMass_Distances.npy')[0]
data2 = np.load('Sys3B_HMass_Distances.npy')[0]
projection1 = tica.transform(data1)
projection2 = tica.transform(data2)

## load the MSM weights

weights = np.concatenate(pickle.load(open(f'./Dtraj_Weights/weights_400_50.pkl','rb')))
...
tica = TICA(lagtime=300, var_cutoff=50/100).fit_fetch(np.concatenate(total_data))
...
l[0].savefig("/home/pdb3/kihongk2/Images/TICA_3A3B.png", dpi=300, transparent = True)
